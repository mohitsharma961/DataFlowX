...
import ExecutionStatusIndicator, { ExecutionStatuLabel } from '@/app/workflow/runs/[workflowId]/_components/ExecutionStatusIndicator';
...
const WorkflowCard = ({workflow}:{workflow:Workflow}) => {
  ...
  return (
    <Card className='border border-seperate shadow-sm rounded-lg overflow-hidden group/card' >
    <CardContent className='p-4 items-center justify-between h-[100px]'>
      <div className='flex items-center justify-start space-x-3'>
          ...
          >
          {isDraft?(<FileTextIcon className="h-5 w-6"/>):(<PlayIcon className='h-5 w-5'/>)}
          </div>
          <div>
     ...
     <div className='flex items-center text-sm gap-2'>
      {lastRunAt && ( <Link href={`/workflow/runs/${workflow.id}/${lastRunId}`} className='flex items-center text-sm gap-2 group'>
        ...
        <ExecutionStatusIndicator status={lastRunStatus as WorkflowExecutionStatus}/>
         <ExecutionStatuLabel status={lastRunStatus as WorkflowExecutionStatus}/>
